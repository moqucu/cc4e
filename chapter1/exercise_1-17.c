...
/* reverse the characters in the given source array */
void reverse_char_array(char sourceArray[], char targetArray[], int charArraySize);
...
int main()
{
    ...
    while ((len = get_line(line, MAXLINE)) > 0)
        if (len > THRESHOLD)
        {
			...
			reverse_char_array(rightZappedLine, reversedLine, MAXLINE);
			
        	printf("%s", rightZappedLine);
        	printf("%s\n", reversedLine);
        }
}
...
void reverse_char_array(char sourceArray[], char targetArray[], int charArraySize)
{
	...
	int maxLength, oldIndex, newIndex;
	
	for (maxLength = 0; maxLength < charArraySize; ++maxLength)
		if (sourceArray[maxLength] == '\0')
			break;
		
	newIndex = 0;
	for (oldIndex = maxLength - 1; oldIndex >= 0; --oldIndex)
	{
		targetArray[newIndex] = sourceArray[oldIndex];
		++newIndex;
	}
	targetArray[maxLength] = '\0';
}